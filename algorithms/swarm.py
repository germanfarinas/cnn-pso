...
class Swarm():
    def __init__(self):
        ...
        self._gbest_v = []
        self._gbest_b = []     # swarm best binary position
        self._gbest_nbf = 0    # number of selected features
    ...
    def _reinit_partial_swarm(self):
        ...
        k = ceil(config.swarm_size * self._swarm_reinit_frac)
        a = np.array([1]*k + [0]*(config.swarm_size-k))
        np.random.shuffle(a)
        # randomize the velocities and positions of the randomly selected particles
        for p in np.array(self._P)[a==1]:
            p.v = np.random.uniform(config.v_min, config.v_max, size=config.particle_size)
    ...
    def _local_search(self):
        """
        Local search algorithm using Random Dependence Coefficient (RDC) to fine tune gbest
        INPUT
            - self:  Uses gbest_b as input, Indexes of selected features ex fs=[3,4] when b=[0,0,0,1,1]
        OUTPUT
            - self: Uses gbest_b as output, reduced feature subset after local search ex f=[3]
        """
        if self._gbest_nbf < config.ls_threshold:
            p = Particle(self)
            p.b = np.ones(config.particle_size)
            for i in range(self._gbest_nbf):
                for j in range(self._gbest_nbf):
                    if i != j:
                        if config.crdc[i, j] == 0: config.crdc[i, j] = \
                            RDC(config.X[:, [i, j]], config.y, k=config.rdc_k, n=config.rdc_n)
                        if config.irdc[i] >= config.irdc[j] \
                                and config.irdc[i] >= max(config.crdc[i, j], config.crdc[j, i]):
                            p.b[j] = 0
                            assert np.shape(p.b) == np.shape(self._gbest_b), \
                                "g_best_b_tmp and g_best_b do not have same shape"
                            p._cost, p._cp = config.fitness_function(p, self)
...
class Particle():
    def __init__(self, sw):
        ...
        self.v = np.random.uniform(config.v_min, config.v_max, size=config.particle_size)
        a = np.array([0] * (config.particle_size - config.feature_init) + [1] * config.feature_init)
        np.random.shuffle(a)
        self.x = np.zeros(config.particle_size)
    ...
    def update_particle(self, sw):
        ...
        self.v = np.clip(self.v, config.v_min, config.v_max)
        self.x = np.clip(self.x + self.v, config.x_min, config.x_max)
        self.b = (sw._R[0] < sigmoid(self.x)).astype(int)
        self._nbf = self.b.sum()
    ...
    def update_pbest(self, sw, data, target):
        #if subset is not empty calculate cost and update pbest
        self._cost, self._cp = config.fitness_function(self, data, target)
        sw._f_calls += 1
        if self._cost < self._pbest_cost:
